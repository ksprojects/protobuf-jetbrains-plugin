...
public class TypeReferenceNodeManipulator extends AbstractElementManipulator<TypeReferenceNode> {
    ...
    @Override
    public TypeReferenceNode handleContentChange(@NotNull TypeReferenceNode node, @NotNull TextRange range, String newContent) throws IncorrectOperationException {
        String oldText = node.getText();
        String newText = oldText.substring(0, range.getStartOffset()) + newContent + oldText.substring(range.getEndOffset());
        Project project = node.getProject();
        PsiFileFactoryImpl factory = (PsiFileFactoryImpl) PsiFileFactory.getInstance(project);
        RuleIElementType type = ProtoParserDefinition.rule(ProtoParser.RULE_typeReference);
